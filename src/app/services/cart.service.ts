...
export class CartService {
  ...
  addItems(item:any){
    const itemExists = this.carReadonly().findIndex((guitar:any)=>guitar.id===item.id); // devuelve el index si encontro el elemento 
  ...
  cartTotal(){
    const total = this.carReadonly().reduce((total:any,item:any)=> total + (item.quantity*item.price),0);
